...
if (!isset($HTTP_POST_VARS['mode']))
{
	if (!isset($HTTP_GET_VARS['action']))
	{
		...
		$template->assign_vars(array(
			'L_ABQ_TITLE' => $lang['ABQ_MOD'] . ' - ' . sprintf($lang['ABQ_Admin_Title'], 1),
			'L_ABQ_EXPLAIN' => $lang['ABQ_Admin_Explain_Type1'],
		...
		else
		{
			...
			$template->assign_vars(array(
				'L_ABQ_NO_QUESTION' => sprintf($lang['ABQ_No_questions'], 1))
			);
		}
	...
	else
	{
		if ($HTTP_GET_VARS['action'] == 'edit')
		{
			...
			$sql = 'SELECT *
				FROM ' . ANTI_BOT_QUEST_TABLE . '
				WHERE id = ' . $abq_id . ' AND imagequestion = \'\'';
			if(!$result = $db->sql_query($sql))
			{
	...
	if ($HTTP_POST_VARS['mode'] == 'new')
	{
		...
		else
		{
			...
			$sql = 'INSERT INTO ' . ANTI_BOT_QUEST_TABLE . ' (id, question, answer1, answer2, answer3, answer4, answer5, anti_bot_img, lang, bbcodeuid, wronganswer01, wronganswer02, wronganswer03, wronganswer04, wronganswer05, wronganswer06, wronganswer07, wronganswer08, wronganswer09, wronganswer10, imagequestion) 
				VALUES (' . $abq_id . ', \'' . $abq_question . '\', \'' . $abq_answer1 . '\', \'' . $abq_answer2 . '\', \'' . $abq_answer3 . '\', \'' . $abq_answer4 . '\', \'' . $abq_answer5 . '\', \'' . $abq_anti_bot_img . '\', \'' . $abq_language . '\', \'' . $bbcode_uid . '\', \'' . $abq_wronganswer01 . '\', \'' . $abq_wronganswer02 . '\', \'' . $abq_wronganswer03 . '\', \'' . $abq_wronganswer04 . '\', \'' . $abq_wronganswer05 . '\', \'' . $abq_wronganswer06 . '\', \'' . $abq_wronganswer07 . '\', \'' . $abq_wronganswer08 . '\', \'' . $abq_wronganswer09 . '\', \'' . $abq_wronganswer10 . '\', \'\')';
			if(!$result = $db->sql_query($sql))
			{
