 ...
 *                            admin_advanced.php
 *                            ------------------
 *   begin                : Saturday, Jan 02, 2005
 *   copyright            : (C) 2005 http://www.community.cback.de
 *   email                : sonny@cback.de
 *
 *   $Id: admin_advanced.php,v 1.0.0 2005/02/01
 *
 *
 ***************************************************************************/
...
// -------------
  ...
  // Welche Bereiche werden aufgerufen?
  if ($_GET[panel] == 'aacp')
    {
		// Ja, das hier ist die neuerung ;)
	...
	if (file_exists($phpbb_root_path.'includes/functions_jr_admin.'.$phpEx))
	{
		$jr_admin_userdata = jr_admin_get_user_info($userdata['user_id']);
		$module = jr_admin_get_module_list($jr_admin_userdata['user_jr_admin']);

		$no_access = '</a> <span style="color: #FF0000"><b>'.$lang['Not_available'].'!!!</b></span>';

		if (!$module['Users']['Ban_Management'] && $userdata['user_level'] <> ADMIN)
		{
			$lang['aacp_kat1_n1'] = $lang['aacp_kat1_n1'] . $no_access;
		}
		if (!$module['Users']['Permissions'] && $userdata['user_level'] <> ADMIN)
		{
			$lang['aacp_kat1_n2'] = $lang['aacp_kat1_n2'] . $no_access;
		}
		if (!$module['Users']['Manage'] && $userdata['user_level'] <> ADMIN)
		{
			$lang['aacp_kat1_n3'] = $lang['aacp_kat1_n3'] . $no_access;
		}
		if (!$module['Groups']['Manage'] && $userdata['user_level'] <> ADMIN)
		{
			$lang['aacp_kat1_n4'] = $lang['aacp_kat1_n4'] . $no_access;
		}
		if (!$module['Groups']['Permissions'] && $userdata['user_level'] <> ADMIN)
		{
			$lang['aacp_kat1_n5'] = $lang['aacp_kat1_n5'] . $no_access;
		}
		if (!$module['General']['Configuration'] && $userdata['user_level'] <> ADMIN)
		{
			$lang['aacp_kat2_n1'] = $lang['aacp_kat2_n1'] . $no_access;
		}
		if (!$module['Forums']['Manage'] && $userdata['user_level'] <> ADMIN)
		{
			$lang['aacp_kat2_n2'] = $lang['aacp_kat2_n2'] . $no_access;
		}
		if (!$module['Forums']['Permissions'] && $userdata['user_level'] <> ADMIN)
		{
			$lang['aacp_kat2_n3'] = $lang['aacp_kat2_n3'] . $no_access;
		}
		if (!$module['Users']['Ranks'] && $userdata['user_level'] <> ADMIN)
		{
			$lang['aacp_kat3_n1'] = $lang['aacp_kat3_n1'] . $no_access;
		}
		if (!$module['General']['Smilies'] && $userdata['user_level'] <> ADMIN)
		{
			$lang['aacp_kat3_n2'] = $lang['aacp_kat3_n2'] . $no_access;
		}
		if (!$module['Styles']['Add_new'] && $userdata['user_level'] <> ADMIN)
		{
			$lang['aacp_kat3_n3'] = $lang['aacp_kat3_n3'] . $no_access;
		}
		if (!$module['Styles']['Manage'] && $userdata['user_level'] <> ADMIN)
		{
			$lang['aacp_kat3_n4'] = $lang['aacp_kat3_n4'] . $no_access;
		}
	}
     ...
     // Template Variablen hinzufügen
  	 $template->assign_vars(array(
  		...
  		'U_AACP_KAT2_N4' => append_sid("../index.$phpEx"),
  		'U_AACP_KAT3_N1' => append_sid("admin_ranks.$phpEx"),
  		'U_AACP_KAT3_N2' => append_sid("admin_smilies.$phpEx"),
  		'U_AACP_KAT3_N3' => append_sid("admin_styles.$phpEx?mode=addnew"),
  		'U_AACP_KAT3_N4' => append_sid("admin_styles.$phpEx"),
  		'U_AACP_KAT4_N1' => append_sid("index.$phpEx?pane=right"),
  ...
  else if ($_GET[panel] == 'phpinfo')
    {
	  ...
	  $ausgabe = preg_replace('#<tr valign="baseline"><td[ ]{0,1}><b>(.*?)</b>#', '<tr><td class="row1" nowrap="nowrap">\1', $ausgabe);
